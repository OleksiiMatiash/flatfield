...
QSharedPointer<Metadata> MetadataReader::readMetadata(const QString& filename)
{
	...
	QList<uint8_t> imageData;
	try {
		image = Exiv2::ImageFactory::open(filename.toStdString(), false);
	}
	catch (...) {
		// Fallback to reading full image, because exiv2 failed to read non-latin char in the path. Maybe will be fixed in future versions of exiv.
		QFile imageFile(filename);
		if (!imageFile.open(QIODeviceBase::ReadOnly))
		{
			return nullptr;
		}
		const int imageSize = imageFile.size();
		imageData.resize(imageSize);

		imageFile.read((char*)imageData.data(), imageSize);

		image = Exiv2::ImageFactory::open(imageData.data(), imageSize);
	}
...
void MetadataReader::removeNewRawImageDigestTag(const QString& filename)
{
	Exiv2::Image::UniquePtr image;
	...
	try
	{
		Exiv2::ExifKey key = Exiv2::ExifKey("Exif.Image.NewRawImageDigest");
